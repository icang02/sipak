...
@section('main-content')
  <div class="main-content container-fluid">
    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-body">
          <form action="{{ route('update.data.dupak', $data->id) }}" method="POST">
            @csrf
            @method('put')
            <div class="row">
              <div class="col-md-6">
                ...
                <div class="form-group">
                  ...
                  <input maxlength="18" style="transition: .3s;" name="nip" type="text"
                    class="form-control @error('nip') is-invalid @enderror" id="nip"
                    value="{{ old('nip', $data->nip) }}" required readonly>
                  @error('nip')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                ...
                <div class="form-group">
                  ...
                  <select style="transition: .3s;" disabled required name="jabatan_id"
                    class="form-select @error('jabatan_id') is-invalid @enderror" id="jabatan_id">
                    <option value="">Pilih jabatan</option>
                    @foreach ($jabatan as $jbt)
                      <option value="{{ $jbt->id }}" @if (old('jabatan_id', $data->jabatan_id) == $jbt->id) selected @endif>
                        {{ $jbt->nama }}</option>
                    @endforeach
                  </select>
                  @error('jabatan_id')
                ...
                <div class="form-group">
                  ...
                  <input style="transition: .3s;" placeholder="-" name="pangkat" type="text"
                    ...
                    value="{{ old('pangkat', $data->pangkat) }}" required readonly>
                  @error('pangkat')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                ...
                <div class="form-group">
                  ...
                  <input style="transition: .3s;" placeholder="-" name="golongan" type="text"
                    ...
                    value="{{ old('golongan', $data->golongan) }}" required readonly>
                  @error('golongan')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <select style="transition: .3s;" disabled name="pak_janjun"
                    class="form-select @error('pak_janjun') is-invalid @enderror" id="pak_janjun">
                    <option value="">Pilih ...</option>
                    @foreach ($timPenilai as $item)
                      <option value="{{ $item->id }}" @if (old('pak_janjun', $data->pak_janjun) == $item->id) selected @endif>
                        {{ $item->nama }}</option>
                    @endforeach
                  </select>
                  @error('pak_janjun')
                ...
                <div class="form-group">
                  ...
                  <select style="transition: .3s;" disabled name="pak_juldes"
                    class="form-select @error('pak_juldes') is-invalid @enderror" id="pak_juldes">
                    <option value="">Pilih ...</option>
                    @foreach ($timPenilai as $item)
                      <option value="{{ $item->id }}" @if (old('pak_juldes', $data->pak_juldes) == $item->id) selected @endif>
                        {{ $item->nama }}</option>
                    @endforeach
                  </select>
                  @error('pak_juldes')
                ...
                <div class="form-group">
                  ...
                  <select style="transition: .3s;" disabled required name="kota"
                    class="form-select @error('kota') is-invalid @enderror" id="kota">
                    <option value="">Pilih kabupaten / kota</option>
                    @foreach ($kab_kota as $item)
                      <option value="{{ $item->name }}" @if (old('kota', $data->kota) == $item->name) selected @endif>
                        {{ $item->name }}</option>
                    @endforeach
                  </select>
                  @error('kota')
                ...
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input style="transition: .3s;" readonly required name="tahun" type="number"
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <div class="form-check">
                        <input disabled id="skp" @if (old('skp', $data->skp) == 1) checked @endif value="1"
                          name="skp" class="form-check-input" type="checkbox" id="skp">
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <div class="form-check">
                        <input disabled id="verifikasi" @if (old('verifikasi', $data->verifikasi) == 1) checked @endif value="1"
                          name="verifikasi" class="form-check-input" type="checkbox" id="verifikasi">
                ...
                <div class="mt-3">
                  <button disabled style="transition: .3s;" id="submit" type="submit"
                    class="btn btn-primary">Update</button>
                  <button style="transition: .3s;" disabled id="reset" type="reset"
  ...
  <script>
    ...
    const skp = document.querySelector('#skp');
    ...
    function modeEdit() {
      ...
      skp.removeAttribute('disabled');
      verifikasi.removeAttribute('disabled');
    ...
    function modeEditOff() {
      ...
      skp.setAttribute('disabled', '');
      verifikasi.setAttribute('disabled', '');
